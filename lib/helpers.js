...
/**
 * Check reserved keywords for name of properties and relations
 * @param {String} name The user input
 * @returns {String|Boolean}
 */
exports.checkReserveWords = function(name) {
  var reserveWords = ['constructor'];
  var conflict = reserveWords.filter(function(rw) {
      return rw===name;
  });
  if(conflict.length>0) {
    return conflict + 'is a reserved keyword. Please use another name';
  }
  return true;
};
